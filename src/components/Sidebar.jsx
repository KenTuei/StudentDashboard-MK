...
// Utility fallback for cn if not using Tailwind's `clsx`/custom helper
const cn = (...classes) => classes.filter(Boolean).join(" ");

// 1. Create Sidebar Context
...
// 4. Sidebar Body (renders both desktop + mobile)
...
export const DesktopSidebar = ({ className, children, ...props }) => {
  ...
  return (
    <motion.div
      className={cn(
        "h-full px-4 py-4 hidden md:flex flex-col bg-neutral-100 dark:bg-neutral-800 w-[300px] shrink-0",
        className
      )}
      animate={{
...
export const SidebarLink = ({ link, className, ...props }) => {
  ...
  const handleClick = (e) => {
    if (link.onClick) {
      ...
      link.onClick();     // Trigger logout or custom action
    }
  };
